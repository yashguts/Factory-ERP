/**
 * Import Front Wall cabin catalog (scripts/front-wall.json) into Supabase.
 * - Ensures door-type sub-categories (ACO/AT/Collapsible/...) under "Front Wall".
 * - Inserts items (mechanical_finished_stock / stocked / make / UOM nos), code FW-NNNN.
...
async function main() {
  const items = JSON.parse(fs.readFileSync(path.join(__dirname, 'front-wall.json'), 'utf8'));
  ...
  const [fw] = await supa(`item_categories?name=eq.Front%20Wall&parent_id=eq.${cabin.id}&select=id&limit=1`);
  if (!fw) throw new Error('Front Wall type not found');
  console.log(`Front Wall type: ${fw.id}`);

  // Ensure a sub-category per distinct door-type Category
  const cats = [...new Set(items.map((it) => it.category))];
  const subId = {};
  for (const c of cats) {
    const ex = await supa(`item_categories?name=eq.${encodeURIComponent(c)}&parent_id=eq.${fw.id}&select=id&limit=1`);
    if (ex.length) subId[c] = ex[0].id;
    else {
      const cr = await supa('item_categories', { method: 'POST', prefer: 'return=representation',
        body: JSON.stringify({ name: c, parent_id: fw.id, procurement_type: 'make' }) });
      subId[c] = cr[0].id;
  ...
  // Existing active names (paged) for idempotency
  ...
  const codes = await supa('items?code=ilike.FW-%25&select=code');
  let maxN = 0;
  codes.forEach((r) => { const m = /^FW-(\d+)$/i.exec(r.code); if (m) maxN = Math.max(maxN, +m[1]); });

  const toInsert = items.filter((it) => !existing.has(it.name.toLowerCase().trim()));
  console.log(`To insert: ${toInsert.length} | skipped (exists): ${items.length - toInsert.length} | starting FW-${maxN + 1}`);
  toInsert.forEach((it, i) => { it.code = `FW-${String(maxN + 1 + i).padStart(4, '0')}`; });
  ...
  for (const batch of chunk(stocked, 200)) {
    ...
    await supa('inventory_transactions', { method: 'POST',
      body: JSON.stringify(batch.map((it) => ({ item_id: idByCode.get(it.code), warehouse_id: MAIN_STORE,
        transaction_type: 'adjustment', quantity: it.stock, notes: 'Opening stock (front wall import)', reference_type: 'import' }))) });
  }
