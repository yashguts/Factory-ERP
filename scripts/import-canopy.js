/**
 * Import Canopy cabin catalog (scripts/canopy.json) into Supabase.
 * - Ensures sub-categories (Fan/Blower/Goods/R1 Type) under "Canopy".
 * - Inserts items (mechanical_finished_stock / stocked / make / UOM nos), code CANOPY-NNN.
 * - Opening stock -> Main Store (inventory + inventory_transactions) for stock != 0 (negatives kept).
...
async function main() {
  const items = JSON.parse(fs.readFileSync(path.join(__dirname, 'canopy.json'), 'utf8'));
  ...
  const [cp] = await supa(`item_categories?name=eq.Canopy&parent_id=eq.${cabin.id}&select=id&limit=1`);
  if (!cp) throw new Error('Canopy type not found');
  console.log(`Canopy type: ${cp.id}`);

  const cats = [...new Set(items.map((it) => it.category))];
  const subId = {};
  for (const c of cats) {
    const ex = await supa(`item_categories?name=eq.${encodeURIComponent(c)}&parent_id=eq.${cp.id}&select=id&limit=1`);
    if (ex.length) subId[c] = ex[0].id;
    else {
      const cr = await supa('item_categories', { method: 'POST', prefer: 'return=representation',
        body: JSON.stringify({ name: c, parent_id: cp.id, procurement_type: 'make' }) });
      subId[c] = cr[0].id;
  ...
  const codes = await supa('items?code=ilike.CANOPY-%25&select=code');
  let maxN = 0;
  codes.forEach((r) => { const m = /^CANOPY-(\d+)$/i.exec(r.code); if (m) maxN = Math.max(maxN, +m[1]); });

  const toInsert = items.filter((it) => !existing.has(it.name.toLowerCase().trim()));
  console.log(`To insert: ${toInsert.length} | skipped (exists): ${items.length - toInsert.length} | starting CANOPY-${maxN + 1}`);
  toInsert.forEach((it, i) => { it.code = `CANOPY-${String(maxN + 1 + i).padStart(3, '0')}`; });
  ...
  for (const batch of chunk(stocked, 200)) {
    ...
    await supa('inventory_transactions', { method: 'POST',
      body: JSON.stringify(batch.map((it) => ({ item_id: idByCode.get(it.code), warehouse_id: MAIN_STORE,
        transaction_type: 'adjustment', quantity: it.stock, notes: 'Opening stock (canopy import)', reference_type: 'import' }))) });
  }
