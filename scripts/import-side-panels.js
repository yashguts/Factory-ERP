/**
 * Import Side Panel cabin catalog (scripts/side-panels.json) into Supabase.
 * - Ensures P1..P6 sub-categories under the "Side Panel" cabin type.
 * - Inserts items (mechanical_finished_stock / stocked / make / UOM nos), code SIDE-NNN.
...
async function main() {
  const items = JSON.parse(fs.readFileSync(path.join(__dirname, 'side-panels.json'), 'utf8'));
  console.log(`Catalog items: ${items.length}`);

  // Cabin parent -> Side Panel type
  const [cabin] = await supa('item_categories?name=eq.Cabin&parent_id=is.null&select=id&limit=1');
  if (!cabin) throw new Error('Cabin parent category not found');
  const [side] = await supa(`item_categories?name=eq.Side%20Panel&parent_id=eq.${cabin.id}&select=id&limit=1`);
  if (!side) throw new Error('Side Panel cabin type not found');
  console.log(`Side Panel type: ${side.id}`);

  // Ensure P1..P6 sub-categories
  const series = ['P1', 'P2', 'P3', 'P4', 'P5', 'P6'];
  const subId = {};
  for (const s of series) {
    const ex = await supa(`item_categories?name=eq.${s}&parent_id=eq.${side.id}&select=id&limit=1`);
    if (ex.length) subId[s] = ex[0].id;
    else {
      const cr = await supa('item_categories', { method: 'POST', prefer: 'return=representation',
        body: JSON.stringify({ name: s, parent_id: side.id, procurement_type: 'make' }) });
      subId[s] = cr[0].id;
      console.log(`  created sub-category ${s}`);
    }
  }
  ...
  if (!nos) throw new Error('UOM nos not found');

  // Existing active item names (paged) for idempotency
  ...
  // Next SIDE-NNN number
  const codes = await supa('items?code=ilike.SIDE-%25&select=code');
  let maxN = 0;
  codes.forEach((r) => { const m = /^SIDE-(\d+)$/i.exec(r.code); if (m) maxN = Math.max(maxN, +m[1]); });

  const toInsert = items.filter((it) => !existing.has(it.name.toLowerCase().trim()));
  console.log(`To insert: ${toInsert.length} | skipped (exists): ${items.length - toInsert.length} | starting code SIDE-${maxN + 1}`);
  toInsert.forEach((it, i) => { it.code = `SIDE-${String(maxN + 1 + i).padStart(3, '0')}`; });

  // Insert items in batches; collect id by code
  ...
  for (const batch of chunk(toInsert, 200)) {
    const payload = batch.map((it) => ({
      ...
      uom_id: nos.id, category_id: subId[it.series], is_active: true,
    }));
    const rep = await supa('items', { method: 'POST', prefer: 'return=representation',
  ...
  // Opening stock -> Main Store (inventory + transaction) for stock != 0
  ...
  for (const batch of chunk(stocked, 200)) {
    ...
    await supa('inventory_transactions', { method: 'POST',
      body: JSON.stringify(batch.map((it) => ({ item_id: idByCode.get(it.code), warehouse_id: MAIN_STORE,
        transaction_type: 'adjustment', quantity: it.stock, notes: 'Opening stock (side panel import)', reference_type: 'import' }))) });
  }
