...
export type AssemblyResult = { ok: true; id: string; warning?: string } | { ok: false; error: string };
...
export async function recordAssemblyRun(input: {
  ...
  if (children.length === 0) return { ok: false, error: "This item has no child parts — nothing to build from." };

  // WARN-BUT-ALLOW (owner's rule): building never blocks on short child stock.
  // We still compute the shortfall and hand it back as a warning so the UI can
  // flag it; the consuming posts are allowed to drive a child negative.
  const stock = await mainStock(supabase, children.map((c) => c.item_id), main);
  const shortIds = children.filter((c) => (stock.get(c.item_id) ?? 0) + 1e-9 < c.qty * qty).map((c) => c.item_id);
  let warning: string | undefined;
  if (shortIds.length) {
    const { data: items } = await supabase.from("items").select("id, code, name").in("id", shortIds);
    const byId = new Map((items ?? []).map((i) => [i.id as string, i]));
    const lines = shortIds.map((id) => {
      const it = byId.get(id);
      const c = children.find((x) => x.item_id === id)!;
      return `${it?.code ?? id} (need ${c.qty * qty}, have ${stock.get(id) ?? 0})`;
    });
    warning = `Built anyway — short on: ${lines.join("; ")}. These child parts are now negative.`;
  }
...
export async function updateAssemblyRunQty(id: string, qty: number): Promise<AssemblyResult> {
  ...
  // WARN-BUT-ALLOW: a qty increase is never blocked on short child stock; the
  // re-sync posts the delta and children may go negative (owner's rule).
