export default function Loading() {
  return (
    <div className="animate-pulse">
      ...
      <div className="h-10 w-[420px] max-w-full bg-[var(--muted)] rounded-lg mb-3" />
      <div className="h-12 w-full bg-[var(--muted)] rounded-lg mb-5" />
      <div className="flex gap-3 mb-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-16 w-32 bg-[var(--muted)] rounded-lg" />
        ))}
      </div>
      <div className="h-9 w-[460px] max-w-full bg-[var(--muted)] rounded mb-4" />
