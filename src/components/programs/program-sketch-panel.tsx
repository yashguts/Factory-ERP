...
/**
 * View / upload / replace / remove the sketch attached to an operation.
 * Mirrors GadDrawingPanel but always has an operationId (detail page), so
 * there's no ensure-id dance. PDFs render via the native viewer; images
 * via <img>.
 */
export function ProgramSketchPanel({
  operationId,
  initialUrl,
  initialFilename,
  initialUploadedAt,
  ...
  const handleRemove = () => {
    if (!confirm("Remove the sketch from this program?")) return;
    startTransition(async () => {
      setError(null);
      try {
        await deleteProgramSketch(operationId);
        setUrl(null);
        setFilename(null);
        setUploadedAt(null);
      } catch (err: unknown) {
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--border)] bg-[var(--muted)]/40 shrink-0">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <Button
            ...
            disabled={busy}
            title={url ? "Replace sketch" : "Upload sketch"}
          >
            {busy ? (
        ...
        {url ? (
          isPdf ? (
            <iframe
              ...
              title={filename ?? "Program sketch"}
            />
          ) : (
            // Arbitrary user-uploaded image of unknown dimensions — a plain
            // <img> is the right tool here (same as the GAD drawing viewer).
            // eslint-disable-next-line @next/next/no-img-element
            <img
              key={url}
              src={url}
              alt={filename ?? "Program sketch"}
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="text-sm font-medium">No sketch yet</p>
            <p className="text-xs text-[var(--muted-foreground)] mt-1 max-w-xs">
              Upload a PDF or image (PNG / JPG / WebP, max 50 MB) of the nest
              layout so anyone can see what this program cuts.
            </p>
            <Button
              size="sm"
              onClick={handlePickFile}
