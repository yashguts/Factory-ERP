-- 019: manual Demand Flow Type override.
--
-- The Demand classification (jobs / formula / none, computed in
-- search_inventory) is an inference; the team needs to correct it per item
-- ("this consumable IS formula-fed, the recipe just isn't captured yet").
-- items.demand_override stores the manual marking; NULL = automatic.
--
-- search_inventory now returns coalesce(override, computed) as demand_source
-- plus a demand_overridden flag so the UI can show which marks are manual.
-- DROP + CREATE because the RETURNS TABLE shape changes (same params, one
-- more output column — old deployed code reads by name and ignores it).

alter table public.items add column if not exists demand_override text
  check (demand_override in ('jobs','formula','none'));
